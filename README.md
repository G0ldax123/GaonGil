...
- 현재 MVP는 실시간 경로 생성이 아니라, 미리 정한 route 후보와 로드뷰 이미지를 AI가 분석하는 구조입니다.
- 경사도 수치 데이터는 사용하지 않고, 프롬프트 기준에 따라 이미지에서 `경사로`를 판단합니다.
- 프론트의 4-1/4-2/4-3 상세 지도는 화면 번호가 아니라 `routeId` 기준으로 매핑됩니다.
